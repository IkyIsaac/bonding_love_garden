...
**Current focus:** Mobile customer experience's core loop is now complete end-to-end: browse plans, pay, get a wristband. Next: wristbands detail screen, memberships/reservations, or start the staff experience.

**Last updated:** Mobile checkout — the real purchase flow behind Plans' "Select Plan": price preview (discount-preview) -> checkout-create-order -> payments-initiate -> a live-updating "waiting for payment" screen watching the order via Realtime -> success/failure. Verified genuinely end-to-end, including the payment-gateway side: since there's no real Selcom sandbox, played Selcom's role directly — after the app called payments-initiate for real, sent a correctly HMAC-SHA256-signed webhook POST to payments-webhook-selcom (same shape and signature scheme the placeholder provider expects), and watched the app transition to "Payment successful" automatically via Realtime the instant the webhook was processed, with zero app-side involvement in the "payment succeeded" half of the story. Did this twice (Full Day Pass, then Half Day Pass), each producing a real paid order, active subscription, and issued wristband, confirmed via direct SQL.
...
- [ ] Wristbands (detail screen), memberships, reservations, notifications
