...
**Current focus:** The mobile customer experience is now feature-complete for every planned screen except notifications: auth, Home, Family, Plans, Wallet, Checkout, Wristbands, Memberships, and Reservations all work against the real backend. Next: notifications, or start the staff experience.

**Last updated:** Mobile Wristbands, Memberships, and Reservations. Wristbands (`features/customer/wristbands/`) shows real scannable QR codes (`qr_flutter`) per family member, matching the `customer_app_wristbands_wallet` mockup. Memberships (`features/customer/memberships/`) lists the family's own subscriptions (distinct from Plans, which is the browsable catalog) with a "Reserve a Game" action on active ones. Reservation booking calls `reservations-book` (game limited to what that subscription's plan actually includes, date/time limited to `reservation_settings.max_advance_days`), then immediately pays the reservation fee inline via the same checkout pipeline built for plans — a reservation fee is a real extra charge on top of the membership, not included in the plan price, and there's no "pay later" affordance since nothing in the schema tracks a reservation's payment status after the fact. Extracted `PaymentStatusView` (the waiting/success/failed flow) out of the plan-checkout screen so both purchase paths share it instead of duplicating the Realtime-watching logic.
