...
**Current focus:** The mobile customer experience is feature-complete except notifications. The staff experience is now fully built out: scan-and-admit, session management, customer registration/approval, customer lookup, staff-assisted selling, and reporting all work end to end against the real backend.

**Last updated:** Mobile staff experience, batch 3 — Reports (`features/staff/reports/`). Per the user's explicit priority order (financial, then user acquisition, then frequency, "plus whatever else is necessary or in the docs"): Financial shows revenue today/this week/this month; New Customers shows signups today/this week/this month plus a pending-approval callout (ties back into Registration); Frequency & Attendance shows sessions today/this week and a "Top Families This Month" ranked-bar list (by visit count) — this last one wasn't in the web admin's own shipped Reports page, but matches the `web_admin_reports_audit_log` mockup's "Top Visitors — By session frequency" panel almost exactly, including its "N visits this month" wording. Top Plans by Revenue and Most Popular Games are carried over from the web admin's existing report, all-time (not date-scoped), matching that page's own behavior exactly. All aggregation happens client-side over a handful of raw queries — same deliberate shortcut the web admin's Reports page itself documents ("pure aggregation belongs in a Postgres view/RPC once order volume justifies it — not worth it yet"), reused rather than reinvented.
