...
**Current focus:** Mobile app (Phase 6) is underway — all four customer bottom-nav tabs (Home, Family, Plans, Wallet) are live against the real backend. Next: checkout (the actual purchase flow behind Plans' "Select Plan").

**Last updated:** Mobile Wallet tab — game credit balance (`family_credit_balance`) + history (`game_credit_ledger`), read-only. Discovered while building it that `wallet-redeem` is staff-only by design (403s for customers) — credits get redeemed by staff scanning a wristband at a game station, not from the app, so there's deliberately no redeem button here, just balance and history.
