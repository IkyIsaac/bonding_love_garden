...
**Current focus:** The mobile customer experience is feature-complete except notifications. Staff experience is underway: `StaffShell` (Home/Scanner/Sessions/Reports) is live with the core front-desk loop — scan-and-admit and active-session management — working end to end against the real backend. Next: Registration (customer approval), Sell, staff Memberships, Reports.

**Last updated:** Mobile staff experience, batch 1 — `StaffShell`, staff Home, Scanner, Sessions. `features/staff/staff_shell.dart` adds the staff bottom nav (Home/Scanner/Sessions/Reports), replacing the router's placeholder. Staff Home (`features/staff/home/`) shows park-wide active-visitor/expiring-soon counts and a recent-admissions feed. Scanner (`features/staff/scanner/`) wraps `mobile_scanner` with an optional "admitting into" game picker and a manual wristband-code fallback (real UX pattern for damaged/unreadable wristbands, also how this was tested here — no physical camera in this environment); both paths call `session-scan-admit` and render its admitted/denied outcome. Sessions (`features/staff/sessions/`) lists every open session park-wide (active/expiring_soon/expired — expired-but-not-yet-closed sessions are deliberately included so staff can close them out) with live countdowns; Extend/End actions call `sessions-manage` and are only shown to supervisor/admin, matching that function's own role check and the `sessions_update` RLS policy. Reports is a placeholder tab for now (kept in the nav so it's a real tab from day one, not a dead route).
...
- [ ] Registration (staff-assisted customer signup + approving pending accounts — the still-missing piece for approving a customer without direct SQL)
- [ ] Sell (staff-assisted checkout, `channel: 'staff_app'`)
- [ ] Staff Memberships (look up a customer's memberships/wristbands during Scanner/Sell flows)
