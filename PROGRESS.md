...
**Current focus:** Mobile app (Phase 6) is underway — customer auth, role-gated routing, Home, Family, and Plans (browsing only) are live against the real backend. Next: checkout (the actual purchase flow behind Plans' "Select Plan"), then Wallet.

**Last updated:** Mobile Plans tab — browsing `access_plans` (Single Visit / Memberships filter) and `packages`, matching the `customer_app_access_plans_offers` mockup, with entry-fee note and included-item counts. Deliberately browsing-only this pass: "Select Plan" opens a "coming soon" dialog rather than a real checkout, which is its own separate pass. No marketing tags ("Most Popular" etc.) since nothing in the schema backs them — reusability over copying the mockup literally.
...
- [ ] Wallet tab (currently a placeholder screen behind real nav)
