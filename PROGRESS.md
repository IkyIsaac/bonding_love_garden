...
**Current focus:** Both the mobile customer experience and the mobile staff experience are now fully feature-complete against the real backend — every screen in `docs/ARCHITECTURE_PLAN.md` §5's mobile app tree is built.

**Last updated:** Customer Notifications (`features/customer/notifications/`). Reads/marks-read the `notifications` table via a Realtime `StreamProvider` (the table was already in the `supabase_realtime` publication) rather than a one-shot fetch, so a notification that arrives while the app is open shows up — badge count and all — with no manual refresh. `BrandedAppBar`'s bell (previously a dead `onPressed: () {}`) now shows an unread-count `Badge` and pushes `/customer/notifications`, but only for `role: customer` — that app bar is shared with the staff shell too, and there's no staff notifications inbox to send a tap to. Found and fixed a real gap while researching this: the only thing that ever wrote a `notifications` row was `notifySupervisorsOfPurchase` (staff-facing, purchase alerts) — nothing wrote a customer-facing one, even though `'payment_confirmed'` is literally named as an example `notifications.type` in that table's own migration comment. Added `notifyCustomerOfPurchase` alongside it in `handlePaymentSucceeded`, notifying the family's `owner_profile_id` (not `orders.buyer_profile_id` — for a staff-app Sell, the buyer is the staff member who rang it up, not the customer).
...
- [x] Notifications (`features/customer/notifications/`) — Realtime-driven inbox, unread badge on `BrandedAppBar`'s bell, mark-read/mark-all-read
...
- [x] Reports (`features/staff/reports/`) — Financial (revenue today/week/month), New Customers (acquisition + pending-approval count), Frequency & Attendance (sessions + top families by monthly visit count), Top Plans by Revenue, Most Popular Games
...
**Staff experience, batch 3 — Reports verification.** Loaded as supervisor against the accumulated real test data from batches 1-2 (six paid orders, five sessions, four customer profiles, one from earlier today) — every figure cross-checked against a direct SQL sum/count matched exactly: revenue today (108,000, four of the six orders), this week/month (183,000, all six — same calendar week and month in this dataset), new customers today (1) vs. this week/month (4), sessions today (1) vs. this week (5), and "Top Families This Month" correctly ranked Michael Mbwana (3 visits) above Fatuma Hassan (2). Top Plans by Revenue and Most Popular Games rendered the same seeded catalog data the web admin's own report shows. No new backend gaps found this time — every table this screen reads from was already staff-readable via existing RLS policies.

**Customer Notifications — verification.** Logged in as the customer test account (`255700000099`) — its `approval_status` had drifted back to `pending` since the last session (not this feature's doing), so it was re-approved via direct SQL the same way as before to unblock a real checkout. Confirmed the empty state first ("No notifications yet." — genuinely true, since `notifyCustomerOfPurchase` didn't exist before this session, so no customer had ever received one). Bought a real Half Day Pass, and the moment the simulated Selcom webhook marked the payment `succeeded`, the bell badge went from nothing to "1" with no app interaction at all — pure Realtime push, confirmed the underlying `notifications` row via SQL first to be sure the badge wasn't coincidental. Opened the inbox, saw the real "Payment confirmed" tile (unread styling: bold, tinted, dot indicator), tapped it, and it flipped to read live — badge cleared, tile un-bolded, "Mark all as read" button disappeared since nothing was left unread.
