        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Expanded(
                  child: Text('Welcome back,\n$firstName', style: Theme.of(context).textTheme.headlineSmall),
                ),
                Container(
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(color: AppColors.surfaceVariant, borderRadius: BorderRadius.circular(AppRadii.base)),
            ...
            Text(
              plan.when(
                data: (p) => p == null ? 'No active membership' : 'Membership: ${p.planName}',
                loading: () => ' ',
              ...
              data: (names) {
                ...
                return Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: AppColors.surfaceVariant, borderRadius: BorderRadius.circular(AppRadii.standard)),
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Family Managed', style: TextStyle(fontWeight: FontWeight.w600)),
                              Text(
                        ...
                        TextButton(onPressed: () => context.go('/customer/family'), child: const Text('Manage')),
                      ],
                    ),
                  ),
                );
...
class _LiveSessionCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return session.when(
      data: (s) => s == null ? const SizedBox.shrink() : Padding(padding: const EdgeInsets.only(bottom: 0), child: _LiveSessionContent(session: s)),
      loading: () => const SizedBox.shrink(),
      error: (_, __) => const SizedBox.shrink(),
    );
  }
...
class _LiveSessionContentState extends State<_LiveSessionContent> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (_) {
      setState(() => _remaining = widget.session.plannedEndAt.difference(DateTime.now()));
    });
  }
  ...
  Widget build(BuildContext context) {
    final total = widget.session.plannedEndAt.difference(widget.session.startedAt).inSeconds;
    final elapsed = DateTime.now().difference(widget.session.startedAt).inSeconds;
    ...
    return Container(
      ...
      padding: const EdgeInsets.all(AppSpacing.sm),
      decoration: BoxDecoration(color: AppColors.secondary, borderRadius: BorderRadius.circular(AppRadii.large)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(AppRadii.full)),
            child: const Text('LIVE PLAYING SESSION', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: AppSpacing.sm),
          const Text('Time Remaining', style: TextStyle(color: Colors.white70)),
          Text('$minutes:$seconds', style: const TextStyle(color: Colors.white, fontSize: 40, fontWeight: FontWeight.bold)),
          const SizedBox(height: AppSpacing.base),
          ClipRRect(
          ...
          Row(
            children: [
              ...
              Text(widget.session.zoneName, style: const TextStyle(color: Colors.white)),
              const SizedBox(width: AppSpacing.sm),
              const Icon(Icons.groups_outlined, size: 16, color: Colors.white),
              const SizedBox(width: 4),
              Text('${widget.session.peopleCount} People', style: const TextStyle(color: Colors.white)),
            ],
          ),
        ],
      ),
...
class _RecentActivityList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return activity.when(
      data: (items) {
        if (items.isEmpty) {
          return Text('No recent activity yet.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.onSurfaceVariant));
        }
        return Column(
          children: items
              .map(
                (item) => Card(
                  ...
                  child: ListTile(
                    leading: const CircleAvatar(backgroundColor: AppColors.primaryContainer, child: Icon(Icons.check, color: AppColors.onPrimaryContainer, size: 18)),
                    title: Text(item.title, style: const TextStyle(fontWeight: FontWeight.w600)),
                    subtitle: item.body != null ? Text(item.body!) : null,
                    trailing: Text(_relativeTime(item.createdAt), style: Theme.of(context).textTheme.bodySmall),
                  ),
                ),
              )
              .toList(),
      ...
      error: (_, __) => Text('Couldn\'t load recent activity.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.error)),
    );
  }
